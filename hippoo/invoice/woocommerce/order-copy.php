...
function populate_order_hippoo_columns( $column, $order ) {
    ...
    switch ($column) {
        case 'order_id_barcode':
            ...
            echo '<img src="' . esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/barcode-scanner.svg') . '" data-src="' . esc_attr($barcode) . '" data-text="' . esc_attr($order_id) . '" class="hippoo-tooltip" />';
            break;
        ...
        case 'order_print':
            echo '<a href="?post_id=' . intval($order_id) . '&download_type=factor" target="_blank" data-barcode="' .
                ...
                esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/invoice-factor.svg') . '" /></a>';
            ...
            echo '<a href="?post_id=' . intval($order_id) . '&download_type=label" target="_blank" data-barcode="' . 
                ...
                esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/shipping-label.svg') . '" /></a>';
            break;
    }
}
...
function add_barcode_value($product) {
    ...
    echo '<td class="barcode" width="1%"><img src="' . esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/barcode-scanner.svg') . '" data-src="' . esc_attr($barcode) . '" data-text="' . esc_attr($sku) . '" class="hippoo-tooltip" /></td>';
}
...
function render_order_barcode_meta_box($post) {
    ...
    echo esc_html($barcode) . '<br><strong>' . esc_html($order_id) . '</strong>';
}

function render_invoice_and_label_meta_box($post) {
    $order_id = $post->ID;

    echo '<a href="?post_id=' . intval($order_id) . '&download_type=factor" target="_blank"><img src="' . esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/invoice-factor.svg') . '" /> Print invoice</a>';
    echo '<br>';
    echo '<a href="?post_id=' . intval($order_id) . '&download_type=label" target="_blank"><img src="' . esc_url(HIPPOO_INVOICE_PLUGIN_URL . 'assets/img/shipping-label.svg') . '" /> Print shipping label</a>';
}
